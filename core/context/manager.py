...
from core.memory import parse_memories
...
class ContextManager:
    ...
    @property
    def global_context_path(self) -> Path:
        """Path to the global Tier 1 CONTEXT.md file."""
        global_dir = Path(self._config.get("global_dir", "~/.mtagent")).expanduser()
        return global_dir / "CONTEXT.md"

    def ensure_global_setup(self) -> bool:
        """确保全局目录和 CONTEXT.md 骨架存在。"""
        global_path = self.global_context_path
        created = False
    ...
    def load(self) -> None:
        ...
        logger.info(
            ...
            len(parse_memories(self._global_context)),
        )
    ...
    def stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "memories_count": len(parse_memories(self._global_context)),
    ...
    def _load_global_context(self) -> str:
        filepath = self.global_context_path
    ...
    def refresh_global_context(self, content: str) -> None:
        """Refresh Tier 1 cache after external updates such as memory writes."""
        self._global_context = content.strip()
        filepath = str(self.global_context_path)
        if self._global_context and filepath not in self._loaded_files:
            self._loaded_files.insert(0, filepath)
    ...
    def _read_file_safe(filepath: Path) -> str:
        try:
            ...
            logger.warning("Failed to read context file %s: %s", filepath, e)
        return ""
